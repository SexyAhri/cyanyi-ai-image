...
import { buildSettingsFromUrlParams, clearUrlSettingParams, hasUrlSettingParams } from '../../lib/api/urlSettings'
...
async function importDefaultConfigOnlyUrlSettings() {
  ...
  return import('../../lib/api/urlSettings')
}
...
describe('URL settings params', () => {
  ...
  it('creates an OpenAI profile from legacy params even when Grok is active', () => {
    const grokProfile = createDefaultGrokProfile({ id: 'grok-active', apiKey: 'grok-key' })
    const current = normalizeSettings({
      ...DEFAULT_SETTINGS,
