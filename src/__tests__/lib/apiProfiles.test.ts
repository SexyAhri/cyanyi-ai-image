...
describe('mergeImportedSettings', () => {
  ...
  it('replaces the default provider list with imported profiles when current settings are untouched', () => {
    const merged = mergeImportedSettings(DEFAULT_SETTINGS, {
      ...
      activeProfileId: 'imported-grok',
    })

    expect(merged.profiles.map((profile) => profile.id)).toEqual(['imported-openai', 'imported-grok'])
    expect(merged.activeProfileId).toBe('imported-grok')
  })
  ...
  it('appends imported profiles as new profiles when current settings are customized', () => {
    ...
    expect(merged.profiles[2]).toMatchObject({ name: 'Imported Grok', provider: 'grok', apiKey: 'grok-key' })
  ...
  it('skips imported profiles that already exist in current customized settings', () => {
    ...
    expect(merged.profiles).toHaveLength(2)
    expect(merged.profiles[0]).toMatchObject({ apiKey: 'current-key', model: 'current-model' })
    expect(merged.profiles[1]).toMatchObject({ provider: 'grok', apiKey: 'grok-key', model: 'grok-image' })
  })
...
\`\`\``)
  ...
  it('does not inherit Grok URL and model when switching to a custom provider', () => {
    const provider = importCustomProviderDefinitionFromJson(JSON.stringify({
      name: 'Custom Provider',
      template: 'http-image',
      submit: { path: 'images/generations' },
    }))
    const profile = switchApiProfileProvider(createDefaultGrokProfile(), provider.id, provider)
  ...
  it('keeps non-OpenAI providers in Images API mode when switching providers', () => {
    ...
    const grokProfile = switchApiProfileProvider(openaiProfile, 'grok')
    const customProfile = switchApiProfileProvider(openaiProfile, provider.id, provider)

    expect(grokProfile).toMatchObject({ provider: 'grok', apiMode: 'images', streamImages: false })
  ...
  it('restores OpenAI-compatible URL after switching through Grok', () => {
    ...
    const grokProfile = switchApiProfileProvider(openaiProfile, 'grok')
    const restoredProfile = switchApiProfileProvider(grokProfile, 'openai')

    expect(grokProfile.baseUrl).toBe(DEFAULT_GROK_BASE_URL)
