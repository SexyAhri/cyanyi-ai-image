...
export function sanitizeProviderErrorMessage(message: string): string {
  ...
  if (
    /no available channel/.test(lower) ||
    /under group/.test(lower) ||
    /渠道|通道|分组|无可用|没有可用/.test(text)
  ) {
    return "当前接口暂时没有可用渠道，请更换 API 配置或稍后重试。";
  }
  ...
  if (
    ...
    return "当前接口暂时无法完成请求，请更换 API 配置或稍后重试。";
  }
    ...
    (set, get) => ({
      ...
      setSettings: (s) =>
        set((st) => {
          ...
          if (!settings.queuePaused) setTimeout(startQueuedTasks, 0);
          return {
            settings,
            ...
            template.id === id
              ? {
                  ...
                  ...(patch.title !== undefined
                    ? {
                        title:
                          patch.title.trim().slice(0, 80) || template.title,
                      }
                    : {}),
...
export function getTaskVersionChain(taskId: string, tasks = useStore.getState().tasks): TaskRecord[] {
  ...
  while (current && !seen.has(current.id)) {
    ...
    const explicitParent = current.parentTaskId ? byId.get(current.parentTaskId) : null;
    if (explicitParent) {
      current = explicitParent;
      continue;
    }
    current = tasks.find((candidate) =>
      candidate.outputImages.some((imageId) => current?.inputImageIds.includes(imageId)),
    ) ?? null;
  }
...
function shouldAutoRetryTaskError(err: unknown): boolean {
...
function getAutoRetryFinalError(retryCount: number) {
...
function scheduleTaskAutoRetry(
  ...
  useStore
    .getState()
    .showToast(`网络连接中断，${Math.round(delay / 1000)} 秒后自动重试`, "info");
...
var DEFAULT_PROMPT_TEMPLATES: PromptTemplate[] = [
  {
  ...
  },
  {
    ...
    title: "局部修改",
    category: "编辑",
    content: "只修改我标注或描述的局部区域，其他区域保持不变，边缘自然融合。",
    createdAt: 0,
    updatedAt: 0,
  },
];
  ...
  return DEFAULT_PROMPT_TEMPLATES ?? [
    {
      ...
      category: "Commercial",
      content:
        "Keep the subject structure, improve lighting, texture, and background for a clean professional product shot.",
      createdAt: 0,
      updatedAt: 0,
    },
    {
      id: "template-character-polish",
      title: "Character polish",
      category: "Character",
      content:
        "Keep identity and composition, enhance facial details, fabric texture, hair, and overall lighting.",
      createdAt: 0,
      updatedAt: 0,
    },
    {
      ...
      category: "Edit",
      content:
        "Only modify the described local area, keep all other areas unchanged, and blend edges naturally.",
      createdAt: 0,
      updatedAt: 0,
    },
  ];
...
function normalizePromptTemplates(value: unknown): PromptTemplate[] {
  const list = Array.isArray(value) ? value : getDefaultPromptTemplates();
  const now = Date.now();
  const seen = new Set<string>();
  return list
...
function buildReducedAgentInput(input: unknown[]): unknown[] {
  ...
  if (
    estimateSerializedSize(noImages) <= AGENT_CONTEXT_REQUEST_HARD_LIMIT_BYTES
  )
    return noImages;
  return noImages;
}
      ...
      for (let i = 0; i < batchItems.length; i++) {
        ...
        if (settled.status === "fulfilled") {
          ...
          outputImages.push({
            ...
            ...(r.error ? { error: sanitizeProviderErrorMessage(r.error) } : {}),
          });
        } else {
...
function startQueuedTasks() {
  ...
  while (runningTaskExecutions.size < settings.queueMaxConcurrency && queuedTaskIds.length > 0) {
    ...
    void runTaskExecution(taskId).finally(() => {
...
function executeTask(taskId: string) {
  ...
  if (settings.queuePaused || runningTaskExecutions.size >= settings.queueMaxConcurrency) {
    updateTaskInStore(taskId, { queued: true });
    return;
  }
  startQueuedTasks();
}
...
export async function retryMultipleTasks(taskIds: string[]) {
  const { tasks, showToast, clearSelection } = useStore.getState();
    ...
    if (options.exportTasks) {
      for (const img of images) {
        ...
        if (thumbnail?.thumbnailDataUrl) {
          ...
          imageFiles[img.id].width =
            imageFiles[img.id].width ?? thumbnail.width;
...
export async function importData(
  ...
  try {
    ...
    if (options.importConfig && data.promptTemplates) {
      const state = useStore.getState();
      useStore.setState({
        promptTemplates: normalizePromptTemplates([
