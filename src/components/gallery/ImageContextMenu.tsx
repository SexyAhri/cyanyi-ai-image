...
import { useStore, addImageFromUrl, ensureImageCached, setTaskOutputAsSeriesReference } from '../../store'
import { copyImageSourceToClipboard, getClipboardFailureMessage } from '../../lib/ui/clipboard'
import { downloadImageEntriesAsZip, downloadImageIds, formatExportFileTime, getImageZipEntries } from '../../lib/gallery/downloadImages'
import { suppressGlobalClicks } from '../../lib/ui/clickSuppression'
import { CopyIcon, DownloadIcon, EditIcon } from '../common/icons'
...
export default function ImageContextMenu() {
  ...
  const handleSetSeriesReference = async (e: React.MouseEvent) => {
    e.stopPropagation()
    const imageId = menuInfo.imageId
    setMenuInfo(null)
    if (!imageId) {
      showToast('没有可设置的图片', 'info')
      return
    }
    const matchedTask = useStore.getState().tasks.find(t => t.outputImages?.includes(imageId))
    if (!matchedTask) {
      const dataUrl = await ensureImageCached(imageId)
      useStore.getState().setSeriesReferenceImage(dataUrl ? {
        id: imageId,
        dataUrl,
        label: '系列基准',
        createdAt: Date.now(),
      } : null)
      showToast(dataUrl ? '已设为系列基准图' : '基准图读取失败，请稍后重试', dataUrl ? 'success' : 'error')
      return
    }
    await setTaskOutputAsSeriesReference(matchedTask, imageId)
  }
  ...
  return (
    ...
    >
      ...
      <button
        onClick={handleSetSeriesReference}
        className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 flex items-center gap-2 transition-colors"
      >
