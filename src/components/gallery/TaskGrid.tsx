...
import { ALL_FAVORITES_COLLECTION_ID, getTaskFavoriteCollectionIds, useStore, reuseConfig, editOutputs, removeTask, setTaskOutputAsSeriesReference, taskMatchesFilterStatus, taskMatchesSearchQuery } from '../../store'
...
export default function TaskGrid() {
  ...
  const markTasksCompared = useStore((s) => s.markTasksCompared)
        ...
        {filteredTasks.map((task) => (
          <div key={task.id} className="task-card-wrapper" data-task-id={task.id}>
            <TaskCard
              ...
              onCompare={() => {
                const ids = selectedTaskIds.includes(task.id)
                  ? selectedTaskIds
                  : [task.id, ...selectedTaskIds]
                if (ids.length < 2) {
                  showToast('请先多选至少 2 个任务再对比', 'info')
                  return
                }
                markTasksCompared(ids)
                showToast(`已标记 ${ids.length} 个任务为对比组`, 'success')
              }}
              onSetSeriesReference={() => void setTaskOutputAsSeriesReference(task)}
