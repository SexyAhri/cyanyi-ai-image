...
import { DEFAULT_PARAMS, type SeriesReferenceSlot, type TaskRecord } from '../../types'
import { getActiveApiProfile, normalizeSettings } from '../../lib/api/apiProfiles'
import { getChangedParams, getOutputImageLimitForSettings, normalizeParamsForSettings } from '../../lib/gallery/paramCompatibility'
import { getAtImageQuery, getImageMentionLabel, getPromptIndexFromVisibleIndex, getPromptMentionParts, getSelectedImageMentionLabel, imageMentionMatches, insertImageMentionAtVisibleRange, insertTextMentionAtVisibleRange, isCursorInSelectedImageMention, stripImageMentionMarkers } from '../../lib/gallery/promptImageMentions'
import {
...
} from '../../lib/input/inputBarMentions'
import { normalizeImageSize } from '../../lib/gallery/size'
...
import { downloadImageEntriesAsZip, downloadImageIds, formatExportFileTime, getTaskOutputImageZipEntries } from '../../lib/gallery/downloadImages'
import Select from '../common/Select'
import SizePickerModal from '../common/SizePickerModal'
import ViewportTooltip from '../common/ViewportTooltip'
import { CloseIcon } from '../common/icons'

const SERIES_SLOT_META: Array<{ id: SeriesReferenceSlot; label: string }> = [
  { id: 'person', label: '人物' },
  { id: 'product', label: '产品' },
  { id: 'style', label: '画风' },
]
...
export default function InputBar() {
  ...
  const seriesReferenceHistory = useStore((s) => s.seriesReferenceHistory)
  const seriesReferenceSlots = useStore((s) => s.seriesReferenceSlots)
  const setSeriesReferenceSlot = useStore((s) => s.setSeriesReferenceSlot)
  const setSeriesReferenceFromSlot = useStore((s) => s.setSeriesReferenceFromSlot)
  ...
  const renderImageThumbs = () => {
    return (
      <div ref={imagesRef}>
        {appMode === 'gallery' && (seriesReferenceImage || seriesReferenceHistory.length > 0) && (
          <div className="mb-3 rounded-2xl border border-indigo-200/80 bg-indigo-50/80 p-3 text-sm text-indigo-950 shadow-sm dark:border-indigo-400/20 dark:bg-indigo-500/10 dark:text-indigo-100">
            {seriesReferenceImage ? (
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  className="h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-white/70 bg-white/70 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.06]"
                  onClick={() => setLightboxImageId(seriesReferenceImage.id)}
                  title="查看系列基准图"
                >
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-xs font-semibold">{seriesReferenceImage.label ?? '系列基准图已启用'}</div>
                  <div className="truncate text-xs opacity-70">后续画廊生成会自动带入这张图作为参考</div>
                </div>
                  ...
                  {SERIES_SLOT_META.map((slot) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        showToast(`已存入固定${slot.label}槽`, 'success')
                      }}
                      className="rounded-full bg-white/70 px-2.5 py-1 text-xs font-medium text-indigo-700 transition hover:bg-white dark:bg-white/[0.08] dark:text-indigo-100"
                    >
                      存{slot.label}
                    </button>
                  ))}
                ...
                <button
                  ...
                  className="rounded-full px-3 py-1.5 text-xs font-medium text-indigo-700 transition hover:bg-white/80 dark:text-indigo-100 dark:hover:bg-white/[0.08]"
                >
                  移除
                </button>
              </div>
            ) : (
              <div className="text-xs font-semibold">系列基准图未启用</div>
            )}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      showToast(`已切换到固定${slot.label}`, 'success')
                    }}
                    className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs transition ${
                      image
                        ? 'bg-white/80 text-indigo-700 hover:bg-white dark:bg-white/[0.08] dark:text-indigo-100'
                        : 'bg-white/40 text-indigo-400 dark:bg-white/[0.04] dark:text-indigo-300/50'
                    }`}
                  >
                    <span className="h-4 w-4 overflow-hidden rounded-full bg-indigo-100 dark:bg-white/[0.08]">
                      {image?.dataUrl && <img src={image.dataUrl} className="h-full w-full object-cover" alt="" />}
                    </span>
                    固定{slot.label}
                  </button>
                )
              ...
              {seriesReferenceHistory.map((image, index) => (
                <button
                  ...
                  className={`h-8 w-8 overflow-hidden rounded-xl border transition ${
  ...
  const renderParams = () => (
    <div className="grid grid-cols-3 gap-2 text-xs flex-1">
      ...
      >
        ...
        <ButtonTooltip visible={false} text="" />
      </label>
      <label
        className="relative flex flex-col gap-0.5"
