...
export async function storeImage(dataUrl: string, source: NonNullable<StoredImage['source']> = 'upload'): Promise<string> {
  const shouldOptimizeForFastWrite = source === 'generated' || source === 'mask'
  const id = shouldOptimizeForFastWrite ? createFastImageId(source) : await hashDataUrl(dataUrl)
  const existing = await getImage(id)
  if (!existing) {
    const metadata: Partial<Omit<StoredImageThumbnail, 'id'>> = shouldOptimizeForFastWrite
      ? await safeReadImageMetadata(dataUrl)
      : await safeCreateImageThumbnail(dataUrl)
    await putImage({
      id,
      dataUrl,
      createdAt: Date.now(),
    ...
    } else if (shouldOptimizeForFastWrite) {
      scheduleImageThumbnailCreation(id, dataUrl)
    }
  } else if ((await getStoredImageThumbnail(id))?.thumbnailVersion !== THUMBNAIL_VERSION) {
    if (shouldOptimizeForFastWrite) {
      scheduleImageThumbnailCreation(id, existing.dataUrl)
    } else {
...
function createFastImageId(source: NonNullable<StoredImage['source']>) {
...
async function readImageMetadata(dataUrl: string): Promise<Pick<StoredImageThumbnail, 'width' | 'height'>> {
  const image = await loadImage(dataUrl)
  const width = image.naturalWidth
  const height = image.naturalHeight
  if (width <= 0 || height <= 0) throw new Error('图片尺寸无效')
...
async function safeReadImageMetadata(dataUrl: string): Promise<Pick<StoredImageThumbnail, 'width' | 'height'>> {
  try {
    return await readImageMetadata(dataUrl)
  } catch {
    return {}
  }
}

function scheduleImageThumbnailCreation(id: string, dataUrl: string) {
  const run = () => {
    void (async () => {
      const existingThumbnail = await getStoredFreshImageThumbnail(id)
      if (existingThumbnail?.thumbnailDataUrl) return

      const image = await getImage(id)
      if (!image) return

      const thumbnail = await safeCreateImageThumbnail(dataUrl)
      if (thumbnail.width && thumbnail.height && (image.width !== thumbnail.width || image.height !== thumbnail.height)) {
        await putImage({ ...image, width: thumbnail.width, height: thumbnail.height })
      }
      if (thumbnail.thumbnailDataUrl) {
        await putImageThumbnail({
