...
import { DEFAULT_PROMPT_STYLE_LOCK_TEXT } from '../gallery/promptStyleLock'
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  const knownProvider = provider === 'gemini' || provider === 'grok' || provider === 'openai' || customProviderIds.has(provider)
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const provider: ApiProvider = rawProvider === 'gemini' || rawProvider === 'grok' || customProviderIds.has(rawProvider) ? rawProvider : 'openai'
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  return {
    ...
    agentModel: typeof record.agentModel === 'string' && record.agentModel.trim() ? record.agentModel : DEFAULT_AGENT_MODEL,
    ...
    promptStyleLockEnabled: typeof record.promptStyleLockEnabled === 'boolean' ? record.promptStyleLockEnabled : true,
    promptStyleLockText: typeof record.promptStyleLockText === 'string' && record.promptStyleLockText.trim()
      ? record.promptStyleLockText
      : DEFAULT_PROMPT_STYLE_LOCK_TEXT,
    profiles,
    activeProfileId,
    galleryProfileId,
    agentProfileId,
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  promptStyleLockText: DEFAULT_PROMPT_STYLE_LOCK_TEXT,
})
