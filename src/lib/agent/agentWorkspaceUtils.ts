...
export function markToolStatusStopped(status: AgentWebSearchStatus): AgentWebSearchStatus {
  if (status.completed) return status
  return { text: status.text.replace(/^正在/, '已停止'), completed: true }
}

export function getImageTaskForOutputItem(item: ResponsesOutputItem, tasksForRound: TaskRecord[]) {
...
export function getBatchImageTasksForOutputItem(item: ResponsesOutputItem, tasksForRound: TaskRecord[]) {
...
export function getTextFromOutputItem(item: ResponsesOutputItem) {
