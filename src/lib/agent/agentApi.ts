...
function createAgentInstructions(settings: AppSettings, useExternalImageTool = false) {
  ...
  const styleLockInstruction = createAgentPromptStyleLockInstruction(settings)
  if (styleLockInstruction) instructions.push('', styleLockInstruction)
...
async function callExternalImageSingle(opts: {
  ...
  const effectivePrompt = applyPromptStyleLock(prompt, settings)
